...
def get_main_menu_keyboard(is_admin: bool = False) -> InlineKeyboardMarkup:
    ...
    # Добавляем основные кнопки в один столбец (width=1)
    builder.row(
        InlineKeyboardButton(text="💳 Продлить подписку", callback_data="open_tariffs"),
...
def get_admin_request_keyboard(request_id: int) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    # Кнопки «Апрув» и «Реджект» в один ряд (width=2 по умолчанию для row)
    builder.row(
...
def get_admin_access_filters_keyboard() -> InlineKeyboardMarkup:
    ...
    # Кнопка назад на отдельную строчку
...
def get_admin_users_keyboard(
    ...
    if nav_row:
        builder.row(*nav_row) # Распаковываем кнопки в один ряд

    # Системные кнопки навигации
    builder.row(
        InlineKeyboardButton(text="⬅️ К фильтрам", callback_data="admin_access_menu"),
        InlineKeyboardButton(text="🏠 В меню", callback_data="back_to_main"),
        width=1
    )
